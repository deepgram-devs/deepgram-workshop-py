"""Deepgram Voice Agent with browser audio, barge-in, and function calling.

This is the finished workshop project -- the end state of Step 8.

Streams microphone audio to Deepgram's Voice Agent WebSocket and plays the
agent's Flux TTS response back, with a browser tab acting as the microphone and
the speaker. The browser handles device selection, permissions, resampling, and
echo cancellation; this file handles the agent.
...
Run it with:  uv run steps/99-final/main.py
Add --local to use the system microphone and speaker instead of a browser.
